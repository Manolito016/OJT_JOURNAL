...
async function handleDownloadWord() {
    if (!downloadReportCache && !aiReportCache) {
        showStatus('No report data available. Please generate a report first.', 'error');
        return;
    }

    const report = downloadReportCache || aiReportCache;
    const { entries, start_date, end_date, student_name = 'Student Name' } = report;

    showStatus('Preparing Word document...', 'success');
    ...
    try {
        ...
        const wordHtml = createWordDocumentHTML(report, imageCache);

        const blob = new Blob(['\ufeff', wordHtml], { type: 'application/msword' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `OJT_Report_${student_name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.doc`;
        ...
        showStatus('Failed to download Word document: ' + error.message, 'error');
    }
}
    ...
    return `
        ...
        <head>
            ...
            <style>
                ...
                .cover-page { text-align: center; page-break-after: always; padding-top: 50px; }
                .cover-college { font-size: 16pt; font-weight: bold; margin: 10px 0; }
                .cover-campus { font-size: 14pt; font-weight: bold; margin: 10px 0 50px 0; }
                .cover-title { font-size: 18pt; font-weight: bold; margin: 30px 0; }
                .cover-company { font-size: 12pt; margin: 20px 0; }
                .cover-name { font-size: 12pt; text-transform: uppercase; font-weight: bold; margin: 50px 0 10px 0; }
                .cover-program { font-size: 12pt; margin: 10px 0; }
                .cover-sy { font-size: 12pt; margin: 10px 0; }
                h1 { font-size: 14pt; font-weight: bold; text-align: center; margin: 30px 0 20px 0; page-break-before: always; }
                h2 { font-size: 13pt; font-weight: bold; margin: 20px 0 10px 0; }
                ...
                .photo-grid { width: 100%; margin-top: 20px; }
                .photo-row { display: flex; flex-wrap: wrap; margin: 10px 0; }
                .photo-item { flex: 1; min-width: 45%; padding: 10px; text-align: center; page-break-inside: avoid; }
        ...
        <body>
            ...
            <div class="cover-page">
                ...
                <p class="cover-company">(Name of Company/office assigned)</p>
                <div style="height: 50px;"></div>
                <p class="cover-name">${escapeHtml(student_name)}</p>
                <p class="cover-program">Bachelor of Science in Information Technology</p>
                <p class="cover-sy">S.Y. 2025 - 2026</p>
            </div>

            <!-- Chapter I -->
            <h1>CHAPTER I: COMPANY PROFILE</h1>
            <h2>Introduction</h2>
            ${chapter1Html || '<p class="placeholder">[Write the introduction of the company here]</p>'}
            
            <h2>Duration and Time</h2>
            <p><em>Start Date: ${start_date}</em></p>
            <p><em>End Date: ${end_date}</em></p>
            
            <h2>Purpose/Role to the Company</h2>
            <p class="placeholder">[Describe your specific role and objectives]</p>

            <!-- Chapter II -->
            <h1>CHAPTER II: IMMERSION DOCUMENTATION</h1>
            <h2>Background of the Action Plan</h2>
            ${chapter2Html || '<p class="placeholder">[Describe the plan you created before starting the immersion]</p>'}
            
            <h2>Program of Activities – Per Day</h2>
            <table>
                <thead>
                    <tr>
                        <th>Day/Date</th>
            ...
            <!-- Chapter III -->
            <h1>CHAPTER III: CONCLUSION AND RECOMMENDATION</h1>
            ${chapter3Html || '<p class="placeholder">[Conclusion and recommendations will appear here]</p>'}

            <!-- Photo Documentation -->
            <h1>APPENDIX: PHOTO DOCUMENTATION</h1>
...
function handleDownloadPdf() {
    ...
    showStatus('Opening print dialog... Select "Save as PDF" as the destination.', 'success');
